...
（有序区，无序区），有序区在开头逐渐生成，两层循环，找到最小的就与无序区第一个元素交换，用到 swap 函数

### 冒泡排序

（无序区，有序区），有序区在后面逐渐生成，两层循环，第二个循环遍历无序区每个元素，每个元素与后面元素比较，后者小则交换；

改进算法，若内部 for 遍历一遍后没有一次交换则跳出整个循环(外循环)；稳定，O(n2)
...
（小于基准区，不确定区(包含基准元素)，大于基准区），选一个数作为基准，将小于基准的数放在基准前，大的放在基准后。使用的代码有两层嵌套 while 循环，并且条件有重复，内层 while 两个循环中的等于条件随便落在哪个循环的条件中

#### 基本步骤

1. 双指针，从两头遍历，左指针寻找大于基准元素的数，右指针寻找小于基准元素的数
2. 左右指针都找到相应的数后，停下来，交换这两个元素
3. 重复执行步骤1,2，直到左右指针指向的元素的位置相同
4. 将步骤3中的元素与基准元素比较，如果基准元素不大于该元素，则交换他们，
  如果大于，则说明右指针从过来没有移动过，基准元素的位置不用动
...
==note==：第二层循环的第一个while的条件总会被否定，但第二个while的条件不一定会被否定，
这就会导致第二层的两个while结束后，`arr[left]`一定大于等于`mid`(除非`left < right`这个条件触发)，
而`arr[right]`一不定小于`mid`

line12，`left++`，如果要交换的话也只要一行代码：`std::swap(arr[left++], arr[end])`。但是你如果仔细观察就会发现`arr[left++]`实际上就是`arr[end]`，理由如下：

在所有的第二层循环中的while结束后，`left==right`，
`arr[left]=arr[right]`可能大于等于`arr[end]`，也可能小于`arr[end]`；
小于时只可能是第二层循环中的第一个while触发了`left < right`这种情况，
这种情况下基准元素的位置不用动

![img](assets/img/2022-05-20-08-11-03.png)

`std::swap(arr[left++], arr[end])`换成`left++`其实就是简化了一下代码，性能好像没有明显的提升。下面给出整个确定基准位置的示意图：

![img](assets/img/2022-05-20-08-10-35.png)
...
- 大根堆和小根堆：大根堆是每个节点的值都大于或等于左右孩子节点的值，对应顺序存储中的规则为：`arr[i]>=arr[2i+i]&&arr[i] >= arr[2i+2]`。小根堆相反：`arr[i] <= arr[2i+1] && arr[i] <= arr[2i+2]`
...
- 向下调整操作：操作对象为一个节点，堆排序在初始化堆后，后续的向下调整操作的节点都是根节点，该操作的示意图如下：
  ...
  ![image-20220403103904512](assets/img/image-20220403103904512.png)

  意思就是说，根节点的左子树和右子树都是大根堆了，唯独根节点对应的树不是，这个时候大根堆的调整只要对根节点执行向下调整即可

#### 堆的初始化

操作对象为一个大根堆的所有父节点(非叶子节点)，操作是向下调整操作
